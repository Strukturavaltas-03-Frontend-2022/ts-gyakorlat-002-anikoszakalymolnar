export default class calculator {
     ...
     */
    users: Array<{ name: string, email: string }> = [{ name: 'Aniko', email: 'aniko@string.com' }, { name: 'Peti', email: 'peti@string.com' }];
    ...
    products: Array<{ name: string, price: number, stock: number }> = [{ name: 'toy', price: 255, stock: 2 }, { name: 'barbie', price: 4555, stock: 1 }];

    /** 
     * Inicializálj egy tuple típust, azaz adj neki értéket is.
     * @type {Tuple} role - number, string, boolean. 
     */
    role: [number, string, boolean] = [1, 'egy', true];
